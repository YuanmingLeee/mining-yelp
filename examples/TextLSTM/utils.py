...
def review_preprocessing(merged_review_csv_dir, glove_embedding_dir, output_dir, train_ratio, keep_ratio, review_length_max, review_length_min, seq_length, embedding_length):
    # split data
    ...
    np.save(join(output_dir, "train_x.npy"), train_x)
    np.save(join(output_dir, "train_y.npy"), train_y)
    np.save(join(output_dir, "test_x.npy"), test_x)
    np.save(join(output_dir, "test_y.npy"), test_y)
...
def sample_review(review_dir, output_dir, n):
    ...
    print("---- num of useful reviews: {}".format(len(useful)))
    ...
    sample_useful = sample(useful, n)
    for text in tqdm(sample_useful):
        output_useful_writer.writerow([text, 1])
    output_useful.close()

    sample_very_useful = sample(very_useful, n)
    for text in tqdm(sample_very_useful):
        output_very_useful_writer.writerow([text, 2])
...
def split_dataset(csv_dir_list, train_ratio):
    ...
    limit = int(len(data) // 3 * train_ratio)
